...
if [ ! -f *-datalist.txt ]; then
    echo "G: -datalist file not found!"
    exit 0
fi

if [ ! -f *.azk ]; then
    echo "G: .azk file not found!"
fi
...
### CORREZIONE PER VERSIONE DIVERSA DI CHECKVOCAL
# esistono due versioni di checkvocal. Una crea una datalist con 
# date e time come prime righe.
# il test che segue fa un check ed eventualmente toglie le prime due righe.


TEST=${arindices[0]}

if [ "$TEST" == 'date' ]; then
	arindices=("${arindices[@]:2}")
	arvalues=("${arvalues[@]:2}")
fi
...
for ((i=0; i < ${#trig_exacttime[@]}; i++)); do
	...
	echo ${arindices[$curr_index]} >> ID_of_ordered_trials.txt
